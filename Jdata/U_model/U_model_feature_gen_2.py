...
def user_features(user, ful_action, sel_action, sep_day):
    dump_path='./cache/user_features_%s_0514_2.csv'%(sep_day)
    if os.path.exists(dump_path):
        actions = pd.read_csv(dump_path)
    
    else:
        sep_day=pd.to_datetime(sep_day)
        day = timedelta(1, 0)
        print('=====> 提取特征...')
...
#         u_feature_history = u_feature_history.drop(['u_click110_history', 'u_click28_history'], axis=1)
        ...
        # 全集行为特征
        action_dummy = pd.get_dummies(ful_action_history['type'], prefix='type')
        action_dummy['type_0'] = action_dummy.sum(axis=1)
        action_dummy = pd.concat([ful_action_history[['user_id', 'sku_id']], action_dummy], axis=1)
        ...
        # 全集行为特征
        action_dummy = pd.get_dummies(ful_action_30D['type'], prefix='type')
        action_dummy['type_0'] = action_dummy.sum(axis=1)
        action_dummy = pd.concat([ful_action_30D[['user_id', 'sku_id']], action_dummy], axis=1)
...
#         u_feature_5D = u_feature_5D.drop(['u_click110_5D', 'u_click28_5D'], axis=1)
        ...
        # 全集行为特征
        action_dummy = pd.get_dummies(ful_action_5D['type'], prefix='type')
        action_dummy['type_0'] = action_dummy.sum(axis=1)
        action_dummy = pd.concat([ful_action_5D[['user_id', 'sku_id']], action_dummy], axis=1)
