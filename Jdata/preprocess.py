...
def concat_action():
    ...
    action.to_csv(path+'/data/JData_Action.csv', index=False)
...
def user_process():
    ...
    user.to_csv(path + '/data/JData_modified_user.csv', index=False)
    
def product_process():
    product = pd.read_csv(path + '/data/JData_Product.csv')
    product = product.drop_duplicates('sku_id')
    product.to_csv(path + '/data/JData_Product.csv', index=False)
    
def action_process():
    product = pd.read_csv(path + '/data/JData_Product.csv')
    user = pd.read_csv(path + '/data/JData_modified_user.csv')
    action = pd.read_csv(path + '/data/JData_Action.csv', parse_dates=[2], infer_datetime_format=True)
    ...
    action.to_csv(path + '/data/JData_subset_action.csv', index=False)
